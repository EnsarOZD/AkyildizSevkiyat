<template>
  <Transition name="fade">
    <div v-if="store.confirmState.isOpen" class="fixed inset-0 z-[60] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div class="inline-block align-bottom bg-white dark:bg-gray-900 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          <div class="bg-white dark:bg-gray-900 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="sm:flex sm:items-start">
              <div class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full sm:mx-0 sm:h-10 sm:w-10"
                :class="iconBgClass">
                <svg v-if="isDanger" class="h-6 w-6 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100" id="modal-title">
                  {{ store.confirmState.options.title || 'Onay Gerekiyor' }}
                </h3>
                <div class="mt-2">
                  <p class="text-sm text-gray-500 dark:text-gray-400">
                    {{ store.confirmState.options.message }}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="bg-gray-50 dark:bg-gray-800 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <button
                ...
                @click="confirm"
            >
              {{ store.confirmState.options.confirmText || 'Evet, Onaylıyorum' }}
            </button>
            <button
                type="button"
                ...
                @click="cancel"
            >
              {{ store.confirmState.options.cancelText || 'İptal' }}
            </button>
          </div>
        </div>
      </div>
...
const isDanger = computed(() => store.confirmState.options.type === 'danger');

const iconBgClass = computed(() => isDanger.value ? 'bg-red-100' : 'bg-yellow-100');
const confirmButtonClass = computed(() => isDanger.value
