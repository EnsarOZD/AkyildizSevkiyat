<template>
  <div class="bg-blue-50 dark:bg-blue-900/10 rounded-2xl border-2 border-blue-100 dark:border-blue-800 shadow-sm overflow-hidden flex flex-col h-full sticky top-6">
    <!-- Header -->
    <div class="px-5 py-4 border-b border-blue-100 dark:border-blue-800 bg-white dark:bg-gray-900 flex items-center justify-between">
    ...
    <div v-if="!hasInitialData" class="flex-1 flex flex-col items-center justify-center p-8 text-center bg-gray-50/50 dark:bg-transparent">
       <div class="h-20 w-20 bg-blue-100 dark:bg-blue-900/20 rounded-full flex items-center justify-center mb-4 animate-pulse">
          <svg class="h-10 w-10 text-blue-300 dark:text-blue-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <!-- Active Form -->
    <div v-else class="flex-1 overflow-y-auto p-5 scrollbar-thin scrollbar-thumb-blue-200">
      <!-- Selected POs display -->
      <div v-if="purchaseOrderIds && purchaseOrderIds.length > 0" class="mb-6 space-y-2">
         <label class="block text-[10px] font-extrabold text-blue-500 uppercase tracking-widest mb-1">Seçili Siparişler ({{ purchaseOrderIds.length }})</label>
         <div class="flex flex-wrap gap-2">
            <div v-for="id in purchaseOrderIds" :key="id" class="inline-flex items-center gap-1.5 bg-white dark:bg-gray-800 border-2 border-blue-100 dark:border-blue-800 rounded-lg px-2.5 py-1 text-xs font-bold text-blue-700 dark:text-blue-300">
      ...
      <div class="space-y-4">
        ...
        <div v-if="!purchaseOrderIds || purchaseOrderIds.length === 0">
          ...
          <select
            ...
            class="w-full border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2.5 text-sm dark:bg-gray-800 dark:text-gray-100 focus:ring-2 focus:ring-blue-500"
          >
        ...
        <div>
          ...
          <div class="relative">
             <input
               ...
               class="w-full border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2.5 text-sm font-bold uppercase placeholder:normal-case dark:bg-gray-800 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 pr-10"
               ref="waybillInput"
             />
             <button @click="$emit('scan')" type="button" class="absolute inset-y-0 right-0 px-3 text-blue-600 hover:text-blue-800 transition-colors" title="OCR Tara">
                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div>
           ...
           <textarea
             ...
             class="w-full border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2 text-sm dark:bg-gray-800 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 resize-none"
           ></textarea>
        </div>
      </div>
    </div>
    ...
    <!-- Footer Action -->
    <div v-if="hasInitialData" class="p-4 bg-white dark:bg-gray-800 border-t border-blue-100 dark:border-blue-800">
       <button
         @click="handleSave"
         :disabled="saving"
         class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-blue-200 dark:shadow-none transition-all flex items-center justify-center gap-2 group active:scale-[0.98]"
       >
