  ...
  <BaseModal :show="isOpen" title="Yeni Satınalma Siparişi" maxWidth="4xl" @close="close">
    <div class="space-y-6">
      ...
      <div class="space-y-4">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm relative z-10">
          <!-- Desktop Header -->
          <div class="hidden md:grid grid-cols-12 gap-4 bg-gray-50/50 dark:bg-gray-800/50 px-6 py-4 border-b border-gray-100 dark:border-gray-800">
             <div class="col-span-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">Ürün Ara</div>
             <div class="col-span-3 text-center text-[10px] font-black text-gray-400 uppercase tracking-widest">Miktar</div>
             <div class="col-span-2 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Birim</div>
             <div class="col-span-1"></div>
          </div>
          
          <!-- Lines -->
          <div class="divide-y divide-gray-100 dark:divide-gray-800">
             <div v-for="(line, index) in form.lines" :key="line.tempId" class="grid grid-cols-1 md:grid-cols-12 md:items-center gap-4 p-4 md:px-6 md:py-4 transition-colors hover:bg-gray-50/50 dark:hover:bg-gray-800/30">
                <div class="col-span-1 md:col-span-6">
                   <label class="md:hidden block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 ml-1">Ürün Ara</label>
                   <StockCombobox v-model="line.stockMasterId" @select="onStockSelect(line, $event)" />
                </div>
                <div class="col-span-1 md:col-span-3">
                   <label class="md:hidden block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 ml-1">Miktar</label>
                   <input 
                ...
                <div class="col-span-1 md:col-span-2 flex items-center justify-between md:justify-start">
                   <label class="md:hidden block text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Birim</label>
                ...
                <div class="col-span-1 md:col-span-1 flex justify-end pt-2 md:pt-0">
                   <button 
                     @click="removeLine(index)" 
                     class="w-full md:w-auto p-2.5 md:p-2 flex items-center justify-center gap-2 text-red-500 bg-red-50 dark:bg-red-900/20 md:text-gray-300 md:bg-transparent md:hover:text-red-500 md:hover:bg-red-50 md:dark:hover:bg-red-900/20 rounded-xl transition-all"
                   >
                      <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                      <span class="md:hidden text-xs font-bold uppercase tracking-wider">Satırı Sil</span>
                   </button>
                </div>
            </div>
         </div>
         
         <div v-if="form.lines.length === 0" class="p-12 text-center text-sm font-bold text-gray-400 italic rounded-b-3xl">
            Henüz kalem eklemediniz.
         </div>
        </div>
      </div>
    </div>
