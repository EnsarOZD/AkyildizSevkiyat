      ...
      <table v-else class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
          <tr v-for="p in filteredProjects" :key="p.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
            ...
            <td class="px-4 py-3 text-gray-700 dark:text-gray-300 max-w-[180px] truncate" :title="p.name">{{ p.name }}</td>
...
const filteredProjects = computed(() => {
  switch (activeTab.value) {
    case 'noCoord':
      return activeProjects.value.filter(p => p.latitude == null || p.longitude == null);
    case 'unvalidated':
      return activeProjects.value.filter(p =>
        p.latitude != null && p.longitude != null && !validations.value.has(p.id)
      );
    case 'compatible':
      return activeProjects.value.filter(p => getValidation(p.id)?.status === 'Compatible');
    case 'suspicious':
      return activeProjects.value.filter(p => getValidation(p.id)?.status === 'Suspicious');
    case 'incompatible':
      return activeProjects.value.filter(p => getValidation(p.id)?.status === 'Incompatible');
    default:
