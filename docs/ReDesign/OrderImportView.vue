<template>
  <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
    ...
    <div class="bg-white dark:bg-gray-900 p-4 rounded shadow">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
        ...
        <div class="flex gap-2 w-full">
            ...
            <button
                @click="checkMappings"
                class="flex-1 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 px-3 py-2 rounded hover:bg-blue-100 dark:hover:bg-blue-900/40 border border-blue-200 dark:border-blue-800 flex items-center justify-center gap-2 text-sm font-medium transition-colors"
            ...
            <button
                @click="showSingleCheck = !showSingleCheck"
                class="flex-1 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 px-3 py-2 rounded hover:bg-blue-100 dark:hover:bg-blue-900/40 border border-blue-200 dark:border-blue-800 flex items-center justify-center gap-1.5 text-sm font-medium transition-colors"
                :class="showSingleCheck ? 'ring-2 ring-blue-400/40' : ''"
                title="Netsis Tekil Sipariş Kontrolü"
            >
    ...
    <!-- Netsis Tekil Sipariş Sorgulama (üstteki "Tekil" butonu ile açılır) -->
    <div v-show="showSingleCheck" class="bg-white dark:bg-gray-900 p-4 rounded shadow">
      ...
      <div class="flex gap-2 items-end">
        <div class="flex-1">
          ...
          <input
            ...
            class="w-full border border-gray-300 dark:border-gray-700 rounded px-3 py-2 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-400"
        ...
        <button
          ...
          class="bg-blue-600 text-white px-4 py-2 rounded text-sm font-semibold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shrink-0"
        >
        ...
        <div class="lg:col-span-3" v-if="activeTab === 'NeedsMapping'">
            <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4 flex items-center justify-between gap-4">
                ...
                <router-link to="/stocks/mappings"
                    class="shrink-0 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-semibold transition-colors whitespace-nowrap">
                    Eşleştirme Sayfası →
                </router-link>
            </div>
        </div>
        ...
        <div class="lg:col-span-3 bg-white dark:bg-gray-900 rounded-lg shadow-sm border dark:border-gray-800">
            ...
            <div class="border-b dark:border-gray-800 flex items-center justify-between">
                <div class="flex overflow-x-auto whitespace-nowrap scrollbar-hide">
                    <button
                        v-for="tab in [{id:'Ready', label:'Hazır', count:readyCount, color:'green'}, {id:'NeedsMapping', label:'Eşleştirme', count:needsMappingCount, color:'red'}, {id:'Passive', label:'Pasif', count:passiveCount, color:'gray'}, {id:'History', label:'Geçmiş', count:null, color:'blue'}]"
                        :key="tab.id"
                                ...
                                <table class="w-full text-left border-collapse">
                                    ...
                                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                        ...
                                        >
                                            ...
                                            <td class="p-4">
                                                <span class="inline-block px-2 py-1 rounded-md bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 font-extrabold text-sm tracking-tight">
                                            ...
                                            <td class="p-4">
                                                ...
                                                <div class="text-xs font-semibold dark:text-gray-200 flex items-center gap-1.5">
                                                    ...
                                                    <span v-if="order.isClothing" class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-violet-50 text-violet-600 dark:bg-violet-900/30 dark:text-violet-300 border border-violet-200 dark:border-violet-800">Kıyafet</span>
                                                </div>
                                                <div class="text-xs text-gray-500">{{ order.projectName || order.ProjectName }}</div>
                                                <div v-if="order.aciklama || order.Aciklama" class="text-[10px] text-blue-600 dark:text-blue-400 truncate max-w-[150px] mt-0.5" :title="order.aciklama || order.Aciklama">
                                            ...
                                            <td class="p-4 text-right">
                                                ...
                                                <div class="sm:hidden flex justify-end gap-2">
                                                     <button @click="openDetail(order)" class="text-xs text-blue-600 font-bold uppercase">Detay</button>
                                                </div>
                                            </td>
                                        </tr>
                                    </tbody>
                                        ...
                                        <div class="flex items-start gap-3">
                                            ...
                                            <div>
                                                <div class="font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2 flex-wrap">
                                                    ...
                                                    <span class="text-sm font-extrabold bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-md tracking-tight">T: {{ order.talepNo || order.TalepNo || '-' }}</span>
                                                </div>
                                                ...
                                                <div v-if="order.aciklama || order.Aciklama" class="text-[10px] text-blue-500 dark:text-blue-400 mt-0.5 line-clamp-1">
                                                    {{ order.aciklama || order.Aciklama }}
                                                </div>
                                            </div>
                                        </div>
                ...
                <div class="p-5 sm:p-6 space-y-5 overflow-y-auto custom-scrollbar">
                     <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 text-sm">
                         ...
                         <div class="col-span-full border-t dark:border-gray-800 pt-4 mt-2">
                             <h4 class="text-xs font-bold text-blue-500 uppercase tracking-widest mb-3">Malzemeler</h4>
                         ...
                         <div class="col-span-full border-t dark:border-gray-800 pt-4 mt-2">
                             <h4 class="text-xs font-bold text-blue-500 uppercase tracking-widest mb-3">İletişim & Teslimat</h4>
                             <div class="space-y-4">
        ...
        <div v-if="showAddressChanges" class="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
            <div class="bg-white dark:bg-gray-900 rounded-xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-xl">
                ...
                <div class="px-5 py-3 border-t dark:border-gray-700 flex justify-between items-center">
                    ...
                    <button @click="showAddressChanges = false" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">Tamam</button>
                </div>
            </div>
        </div>
    </div>
...
const showSingleCheck = ref(false);
