...
{
    /// <summary>
    /// Bir bölge hazırlığının (zone) sevkiyatları için Netsis'ten irsaliye numaralarını çeker
    /// ve tüm aktarılmış sevkiyatların irsaliyesi geldiğinde <c>IrsaliyeFetched</c> bayrağını set eder.
    /// Hem "Netsisten İrsaliye Çek" butonu (FetchZoneIrsaliyeCommand) hem de arka plan servisi
    /// (depocular unutsa bile otomatik ilerlemesi için) bu servisi kullanır. Auth/durum guard'ı içermez.
    /// </summary>
    public class ZoneIrsaliyeFetcher
    {
        private readonly IApplicationDbContext _context;
        private readonly INetsisClient _netsisClient;

        public ZoneIrsaliyeFetcher(IApplicationDbContext context, INetsisClient netsisClient)
        {
            _context = context;
            _netsisClient = netsisClient;
        }

        public async Task<FetchZoneIrsaliyeResult> FetchAsync(int zonePreparationId, CancellationToken cancellationToken)
        {
            var errors   = new List<string>();
            var warnings = new List<string>();

            var zp = await _context.ZonePreparations
                .FirstOrDefaultAsync(z => z.Id == zonePreparationId, cancellationToken);
            if (zp == null)
            {
                errors.Add($"Bölge hazırlığı bulunamadı (#{zonePreparationId}).");
                return new FetchZoneIrsaliyeResult(0, 0, errors, warnings);
            }
            ...
            // Tüm aktarılmış sevkiyatların irsaliyesi geldiyse bölgeyi "irsaliye çekildi" olarak işaretle
